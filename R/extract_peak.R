...
extract_peak <- function(peak) {
  rtr <- df_peak[peak,] |>
    dplyr::mutate(rtmin = (rt_min + CAD_SHIFT) * 60,
  ...
  ms_chr <- dda_data_min |>
    ...
    MSnbase::chromatogram(rt = rtr,
                          mz = mzr)
  
  ms_ready <-
    data.frame(intensity = ms_chr[1]@intensity,
  ...
  ms_peak <-
    MSnbase::Chromatogram(intensity = ms_ready$intensity,
