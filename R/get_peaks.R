...
get_peaks <- function(chrom_list,
  ...
  convert_indices_to_times <- function(x, chrom_list, idx, tfac) {
    ...
    x[, c("rt", "start", "end")] <- sapply(c("rt", "start", "end"), function(j) {
      timepoints[x[, j]]
    })
  ...
  result <- lapply(seq_along(chrom_list), function(sample) {
    suppressWarnings(ptable <- lapply(lambdas, function(lambda) {
      fit_peaks <- function(x,
        ...
        get_lambda_idx <- function(lambda, lambdas, y, allow_max = TRUE) {
          if (lambda == "max") {
            ...
            lambda.idx <- ifelse((length(lambdas) == 1), 1, which(lambdas == as.numeric(lambda)))
          }
        ...
        if (is.null(pos)) {
          find_peaks <- function(y,
            ...
            if (smooth_type == "savgol") {
              ...
              d <- caTools::runmean(caTools::runmean(diff(y), k = smooth_window), k = smooth_window)
            } else {
              d <- diff(y)
            }
            ...
            if (bounds) {
              ...
              # find lower bound
              suppressWarnings(bl <- sapply(p, function(v) {
                max(p4[p4 < v])
              }))
              bl[which(bl == -Inf)] <- 1

              # find upper bound
              suppressWarnings(bu <- sapply(p, function(v) {
                min(p4[p4 > v])
              }))
        ...
        fitpk_egh <- function(x,
          ...
          fit_egh <- function(x1,
            ...
            egh <- function(x,
              ...
              result[index] <- height * exp(-(x[index] - center)^2 / (2 * width^
                2 + tau * (x[index] - center)))
              return(result)
            }
      ...
      pks <- convert_indices_to_times(pks,
