...
```{r check-alignment-default, message=FALSE, warning=FALSE}
check_chromatograms_alignment(show_example = TRUE)
```

Or, with more details:

```{r check-alignment-2, message=FALSE, warning=FALSE}
...
```{r check-alignment-3, message=FALSE, warning=FALSE}
check_chromatograms_alignment(
  show_example = TRUE,
  time_min = 0.5,
  time_max = 35.0,
...
```{r check-integration-1, message=FALSE, warning=FALSE}
...
```{r check-integration-2, message=FALSE, warning=FALSE}
check_peaks_integration(
  show_example = TRUE
)
```

We now recommend you to read the [next vignette](https://adafede.github.io/cascade/vignettes/articles/II-processing.html).
